...
class Agent(object):
    ...
    def get_redq_q_target_no_grad(self, obs_next_tensor, rews_tensor, done_tensor):
        num_mins_real = self.num_min
        floored_num_mins = np.floor(self.num_min)
        if self.num_min - floored_num_mins > 0.001:
            prob_for_higher_value = self.num_min - floored_num_mins
            if np.random.uniform(0, 1) < prob_for_higher_value:
                num_mins_real = int(floored_num_mins+1)
            else:
                num_mins_real = int(floored_num_mins)
        num_mins_real = int(num_mins_real)
        sample_idxs = np.random.choice(self.num_Q, num_mins_real, replace=False)
        with torch.no_grad():
            ...
            if self.q_target_mode == 'rem':
                ...
                y_q = rews_tensor + self.gamma * (1 - done_tensor) * next_q_with_log_prob
            if self.q_target_mode == 'minpair':
                """ATTENTION!! we simply take num_Q as 10, num_mins as 2 and use one case to reproduce the experiment"""
                pair_sample = np.array([[0, 5],[1,6],[2,7],[3,8],[4,9]])
                sample_idxs_pair = np.random.choice(int(self.num_Q / 2), 1, replace=False)
                sample_idxs = pair_sample[sample_idxs_pair]
                a_tilda_next, _, _, log_prob_a_tilda_next, _, _ = self.policy_net.forward(obs_next_tensor)
                q_prediction_next_list = []
                for sample_idx in sample_idxs[0]:
                    q_prediction_next = self.q_target_net_list[sample_idx](torch.cat([obs_next_tensor, a_tilda_next], 1))
                    q_prediction_next_list.append(q_prediction_next)
                q_prediction_next_cat = torch.cat(q_prediction_next_list, 1)
                min_q, min_indices = torch.min(q_prediction_next_cat, dim=1, keepdim=True)
            ...
            if self.q_target_mode == 'weighted':
                ...
                for sample_idx in sample_idxs:
                    ...
                    q_prediction_next_list.append(q_prediction_next)
                q_prediction_next_cat = torch.cat(q_prediction_next_list, 1)
                """compulate the mean instead of min"""
                mean_q = torch.mean(q_prediction_next_cat, dim=1, keepdim=True)
                next_q_with_log_prob = mean_q - self.alpha * log_prob_a_tilda_next
                y_q = rews_tensor + self.gamma * (1 - done_tensor) * next_q_with_log_prob
    ...
    def train(self, logger):
        ...
        num_update = 0 if self.replay_buffer.size <= self.delay_update_steps else self.utd_ratio
